...
public class vit : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        ...
        while (selectm.Read())
        {
            IDataReader selectd = dbConnector.Select("SELECT vit_c FROM click");
            ...
            while (selectd.Read())
            {
                ...
                if (selectd.GetInt32(0) <= selectm.GetInt32(0))
                {
                    ...
                    string updateQuery;
                    string updateQuery1;
                    IDataReader selectAllFromPlayer = dbConnector.Select("SELECT vit FROM postac");
                    while (selectAllFromPlayer.Read())
                    {
                        b = selectAllFromPlayer[0].ToString();
                        a = Int32.Parse(b);
                        a++;
                        updateQuery = "UPDATE postac SET vit = " + a;
                        //Debug.Log(updateQuery);
                        dbConnector.UpdateDB(updateQuery);
                    }

                    IDataReader selectAllFromPlayer1 = dbConnector.Select("SELECT vit_c FROM click");
                    while (selectAllFromPlayer1.Read())
                    {
                        ...
                        updateQuery1 = "UPDATE click SET vit_c = " + a1;
